...
export default function UbicacionesPage() {
  ...
  const [editTipo, setEditTipo] = useState("deposito");
  const [editParent, setEditParent] = useState("");
  const [porBorrar, setPorBorrar] = useState<Ubicacion | null>(null);
  ...
  async function guardarEdicion(u: Ubicacion) {
    ...
    const r = await fetch(`/api/inventario/ubicaciones/${u.id}`, {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ nombre, tipo: editTipo, parent_id: editParent || null }),
    });
    const j = await r.json();
  ...
  async function borrar(u: Ubicacion) {
    ...
    try {
      ...
      // El servidor explica por qué no se puede (stock, movimientos, hijas).
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {itemsOrdenados.map((u) => {
                ...
                if (editId === u.id) {
                  return (
                    <tr key={u.id} className="bg-[#4FAEB2]/5">
                      ...
                      <td className="px-4 py-2">
                        <select
                          ...
                          aria-label="Ubicación padre"
                          className="w-full rounded-lg border border-slate-200 px-2.5 py-1.5 text-sm outline-none focus:border-[#4FAEB2]"
                        >
                          <option value="">— ninguna —</option>
                          {items
                            .filter((o) => o.id !== u.id && o.parent_id == null)
                            .map((o) => (
                  ...
                  <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <button
                          onClick={() => { setPorBorrar(u); setError(null); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Solo se puede borrar una ubicación vacía y sin historial. Si tiene stock, movimientos de
              inventario o sub-ubicaciones, el sistema lo va a impedir y te va a decir cuántos son.
            </p>
            <p className="mt-2 text-sm text-slate-600">
