 ...
 *
 * Requiere en Supabase: Settings → API → "Exposed schemas" incluir `distribuidorajmerp`
 * (además de lo que ya tengas para auth/storage).
 */
export const SUPABASE_APP_SCHEMA: string = process.env.APP_DB_SCHEMA?.trim() || "distribuidorajmerp";
 ...
 * - `null`, `undefined` o string vacío → legado: datos en plantilla compartida `distribuidorajmerp`.
 *
...
/**
 * Cliente Supabase con cualquier esquema PostgREST (`distribuidorajmerp`, `erp_*`, etc.).
